...
import { Phone, Mail } from "lucide-react"; // Essential for the icons
          ...
          {translations.firm.members.map((member, index) => (
            ...
            >
              ...
              <div className="flex flex-col gap-2 mb-6 text-slate-300 text-sm">
                {member.phone && (
                  <a href={`tel:${member.phone}`} className="flex items-center gap-2 hover:text-[#C9A96E] transition-colors w-fit">
                    <Phone size={14} className="text-[#C9A96E]" />
                    {member.phone}
                  </a>
                )}
                {member.email && (
                  <a href={`mailto:${member.email}`} className="flex items-center gap-2 hover:text-[#C9A96E] transition-colors w-fit">
